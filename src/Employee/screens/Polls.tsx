...
export default function EmployeePolls() {
  ...
  // Fetch comments dynamically for selected poll
  useEffect(() => {
    if (selectedPollId) {
      apiClient.get<any>(`/api/polls/${selectedPollId}/comments`)
        .then(res => {
          setComments(res.data.items || res.data.comments || []);
        })
        .catch(err => {
  ...
  const pollComments = comments; // comments are pre-filtered by selectedPollId from the server
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiClient.get<any>(`/api/polls/${selectedPollId}/comments`);
      setComments(res.data.items || res.data.comments || []);
    } catch (err: any) {
