...
import { Plus, Search, MoreHorizontal, Eye, Edit, Trash2, Bell, AlertTriangle } from "lucide-react";
...
export default function Notifications() {
  ...
  const [itemsList, setItemsList] = useState<LegalNotification[]>([]);
  ...
  const [selectedItem, setSelectedItem] = useState<LegalNotification | null>(null);
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const list = await listResource<LegalNotification>("legal/notifications");
      setItemsList(list);
  ...
  const handleAdd = async () => {
    try {
      ...
      await createResource<LegalNotification>("legal/notifications", payload);
      await loadData();
      setAddOpen(false);
      resetForm();
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      await updateResource<LegalNotification>("legal/notifications", selectedItem.id, payload);
      await loadData();
      setEditOpen(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsSubmitting(true);
      await deleteResource("legal/notifications", selectedItem.id);
      await loadData();
      setDeleteOpen(false);
      setSelectedItem(null);
  ...
  return (
    <motion.div className="pl-12 pr-2 sm:pr-0 pb-6 space-y-6" variants={containerVariants} initial="hidden" animate="visible">
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-blue-400">
              ...
              <h1 className="text-2xl font-bold tracking-tight text-white">Legal Notifications</h1>
            </div>
            <p className="text-sm text-slate-400 max-w-2xl">Manage your notifications and associated metadata.</p>
          </div>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="bg-[#0f172a] border-white/10 text-white sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Edit Notification {selectedItem?.notificationNumber}</DialogTitle></DialogHeader>
          <div className="py-4">
            {apiError && <div className="mb-4 p-3 bg-red-500/10 border border-red-500/20 text-red-400 rounded-md text-sm">{apiError}</div>}
            {renderFormFields()}
          </div>
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent className="bg-[#0f172a] border-rose-500/20 text-white sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-rose-500 flex items-center gap-2"><AlertTriangle className="h-5 w-5" /> Delete Notification</DialogTitle>
            <DialogDescription className="text-slate-400 mt-2">Are you sure you want to delete <strong>{selectedItem?.notificationNumber}</strong>? This cannot be undone.</DialogDescription>
          </DialogHeader>
