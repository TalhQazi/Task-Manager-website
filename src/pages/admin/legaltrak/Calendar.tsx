...
export default function Calendar() {
  ...
  const [itemsList, setItemsList] = useState<LegalCalendar[]>([]);
  ...
  const [selectedItem, setSelectedItem] = useState<LegalCalendar | null>(null);
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const list = await listResource<LegalCalendar>("legal/calendar");
      setItemsList(list);
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      await updateResource<LegalCalendar>("legal/calendar", selectedItem.id, payload);
      await loadData();
      setEditOpen(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsSubmitting(true);
      await deleteResource("legal/calendar", selectedItem.id);
      await loadData();
      setDeleteOpen(false);
      setSelectedItem(null);
  ...
  return (
    <motion.div className="pl-12 pr-2 sm:pr-0 pb-6 space-y-6" variants={containerVariants} initial="hidden" animate="visible">
            ...
            <DialogContent className="bg-[#0f172a] border-white/10 text-white sm:max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button onClick={handleAdd} disabled={isSubmitting} className="bg-blue-600 hover:bg-blue-500 text-white">
                  {isSubmitting ? "Saving..." : "Create Calendar"}
                </Button>
              </div>
            </DialogContent>
          </Dialog>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="bg-[#0f172a] border-white/10 text-white sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Edit Calendar {selectedItem?.calendarNumber}</DialogTitle></DialogHeader>
          <div className="py-4">
            {apiError && <div className="mb-4 p-3 bg-red-500/10 border border-red-500/20 text-red-400 rounded-md text-sm">{apiError}</div>}
            {renderFormFields()}
          </div>
      ...
      <Dialog open={viewOpen} onOpenChange={setViewOpen}>
        <DialogContent className="bg-[#0f172a] border-white/10 text-white sm:max-w-lg">
          <DialogHeader><DialogTitle>Calendar Details</DialogTitle></DialogHeader>
          {selectedItem && (
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent className="bg-[#0f172a] border-rose-500/20 text-white sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-rose-500 flex items-center gap-2"><AlertTriangle className="h-5 w-5" /> Delete Calendar</DialogTitle>
            <DialogDescription className="text-slate-400 mt-2">Are you sure you want to delete <strong>{selectedItem?.calendarNumber}</strong>? This cannot be undone.</DialogDescription>
          </DialogHeader>
