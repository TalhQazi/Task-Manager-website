...
import { Plus, Search, MoreHorizontal, Eye, Edit, Trash2, ListTodo, AlertTriangle } from "lucide-react";
...
export default function Tasks() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const list = await listResource<LegalTask>("legal/tasks");
      setItemsList(list);
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      await updateResource<LegalTask>("legal/tasks", selectedItem.id, payload);
      await loadData();
      setEditOpen(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsSubmitting(true);
      await deleteResource("legal/tasks", selectedItem.id);
      await loadData();
      setDeleteOpen(false);
      setSelectedItem(null);
  ...
  const renderFormFields = () => (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div className="space-y-1"><label className="text-xs font-medium text-slate-300">Task *</label><input type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full rounded-md border border-white/10 bg-white/5 px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50" placeholder="Task" required /></div>
      ...
      <div className="space-y-1"><label className="text-xs font-medium text-slate-300">Status </label><select value={formData.status} onChange={e => setFormData({...formData, status: e.target.value as any})} className="w-full rounded-md border border-white/10 bg-[#1e293b] px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50"><option value="To Do">To Do</option><option value="In Progress">In Progress</option><option value="Done">Done</option></select></div>
  ...
  return (
    <motion.div className="pl-12 pr-2 sm:pr-0 pb-6 space-y-6" variants={containerVariants} initial="hidden" animate="visible">
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-blue-400">
              ...
              <h1 className="text-2xl font-bold tracking-tight text-white">Legal Tasks</h1>
            </div>
            <p className="text-sm text-slate-400 max-w-2xl">Manage your tasks and associated metadata.</p>
          </div>
          ...
          <Table>
            <TableHeader className="bg-black/20 hover:bg-black/20">
              <TableRow className="border-white/10 hover:bg-transparent">
                <TableHead className="text-slate-400">Task No.</TableHead>
              ...
              {loading ? (
                ...
                filteredItems.map((c) => (
                  <TableRow key={c.id} className="border-white/10 hover:bg-white/5 transition-colors">
                    <TableCell className="font-mono text-sm text-slate-300">{c.taskNumber}</TableCell>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="bg-[#0f172a] border-white/10 text-white sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Edit Task {selectedItem?.taskNumber}</DialogTitle></DialogHeader>
          <div className="py-4">
            {apiError && <div className="mb-4 p-3 bg-red-500/10 border border-red-500/20 text-red-400 rounded-md text-sm">{apiError}</div>}
            {renderFormFields()}
          </div>
          ...
          {selectedItem && (
            <div className="py-4 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div><div className="text-xs text-slate-400">Task No.</div><div className="font-mono text-lg">{selectedItem.taskNumber}</div></div>
                <div><div className="text-xs text-slate-400">Task</div><div className="font-medium">{selectedItem.title || 'N/A'}</div></div>
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent className="bg-[#0f172a] border-rose-500/20 text-white sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-rose-500 flex items-center gap-2"><AlertTriangle className="h-5 w-5" /> Delete Task</DialogTitle>
            <DialogDescription className="text-slate-400 mt-2">Are you sure you want to delete <strong>{selectedItem?.taskNumber}</strong>? This cannot be undone.</DialogDescription>
          </DialogHeader>
