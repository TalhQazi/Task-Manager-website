...
export default function AdminPolls() {
  ...
  // Fetch details dynamically for selected poll
  useEffect(() => {
    if (selectedPollId) {
      // 1. Fetch Comments
      apiClient.get<any>(`/api/polls/${selectedPollId}/comments`)
        .then(res => setComments(res.data.items || res.data.comments || []))
        .catch(err => console.error("Comments fetch error:", err));

      // 2. Fetch Votes
      apiClient.get<any>(`/api/polls/${selectedPollId}/votes`)
        .then(res => setVotes(res.data.items || res.data.votes || []))
        .catch(err => console.error("Votes fetch error:", err));

      // 3. Fetch Audit Logs
      apiClient.get<any>(`/api/polls/${selectedPollId}/audit-logs`)
        .then(res => setAuditLogs(res.data.items || res.data.auditLogs || []))
  ...
  const handleCreatePollSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const created = await createPollAction(payload);
  ...
  const handleRecordDecisionSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await recordDecisionAction(selectedPollId, decisionText, decisionStatus, "Super Admin");
      setDecisionText("");
      await loadData();
    } catch (err: any) {
  ...
  const handleClosePoll = async () => {
    ...
    try {
      await updatePollStatusAction(selectedPollId, "Closed", "Super Admin");
      await loadData();
    } catch (err: any) {
      toast.error(err.message);
    }
