...
export default function ManagerPolls() {
  ...
  // Fetch votes dynamically for selected poll
  useEffect(() => {
    if (selectedPollId) {
  ...
  const triggerNudge = async (employeeName: string, email: string) => {
    if (!selectedPollId) return;
    try {
      await apiClient.post(`/api/polls/${selectedPollId}/nudge`);
      toast.success(`Nudge dispatched! Email reminder and push alert sent to ${employeeName} (${email}).`);
    } catch (err: any) {
      toast.error(err.message || "Failed to dispatch nudge");
    }
  };

  const triggerDepartmentNudge = async () => {
    if (!selectedPollId) return;
    try {
      await apiClient.post(`/api/polls/${selectedPollId}/nudge`);
      toast.success("Dispatched department-wide reminder nudges to all outstanding voters.");
    } catch (err: any) {
      toast.error(err.message || "Failed to trigger department nudge");
    }
  };
