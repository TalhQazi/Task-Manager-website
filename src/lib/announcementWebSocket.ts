// src/lib/announcementWebSocket.ts
/**
 * Socket.IO integration for real-time announcement updates.
 */
...
/**
 * Wrapper around a Socket.IO client to provide the same handler interface
...
class AnnouncementWebSocket {
  ...
  /** Resolve the host for the WS connection. */
  private resolveHost(): string {
    return (typeof process !== "undefined" && process?.env?.NEXT_PUBLIC_WS_HOST)
  ...
  connect(handlers: WebSocketHandlers) {
    ...
    this.socket.on("connect", () => {
      console.log("[Announcements WS] Connected");
      this.reconnectAttempts = 0;
    });

    this.socket.on("disconnect", () => {
      console.log("[Announcements WS] Disconnected");
      this.attemptReconnect();
    });

    this.socket.on("error", (error) => {
      console.error("[Announcements WS] Error:", error);
      this.handlers.onError?.(error);
    });

    // Register announcement‑related events
  ...
  private attemptReconnect() {
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      console.log(`[Announcements WS] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts})`);
      setTimeout(() => {
      ...
      console.error("[Announcements WS] Max reconnection attempts reached");
    }
  }

  /** Send a generic message payload to the server. */
  send(data: any) {
